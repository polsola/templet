 ...
 */
function u_ecommerce_scripts() {
	$theme   = wp_get_theme();
	$version = $theme->get( 'Version' );

	wp_register_script( 'ecommerce', u_STATIC . '/scripts/ecommerce.js', null, $version, true );
	wp_enqueue_script( 'ecommerce' );

}
add_action( 'wp_enqueue_scripts', 'u_ecommerce_scripts', 0 );

/**
 * Add WooCommerce support for theme
 */
function u_woocommerce_support() {
	add_theme_support( 'woocommerce' );
	add_theme_support( 'wc-product-gallery-zoom' );
	add_theme_support( 'wc-product-gallery-lightbox' );
	add_theme_support( 'wc-product-gallery-slider' );
}
add_action( 'after_setup_theme', 'u_woocommerce_support' );
...
function u_ecommerce_header_items() {
	?>
...
add_action('u_header_main', 'u_ecommerce_header_items');


/**
 * Change number or products per row to 3
 */
function u_loop_columns() {
	return 3; // 3 products per row
}
add_filter( 'loop_shop_columns', 'u_loop_columns' );
 ...
 */
function u_related_products_args( $args ) {
	$args['posts_per_page'] = 3;
	$args['columns'] = 3;
	return $args;
}
add_filter( 'woocommerce_output_related_products_args', 'u_related_products_args' );
