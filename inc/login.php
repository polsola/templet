...
add_action( 'login_enqueue_scripts', 'u_login_stylesheet' );

/**
 * Change login url to WordPress home
 */
function u_login_logo_url() {
	return home_url();
}
add_filter( 'login_headerurl', 'u_login_logo_url' );

/**
 * Change login url title attribute to WordPress site title
 */
function u_login_logo_url_title() {
	return get_bloginfo( 'title' );
}
add_filter( 'login_headertext', 'u_login_logo_url_title' );
 ...
 */
function u_redirect_users_by_role() {
...
// add_action( 'admin_init', 'u_redirect_users_by_role' );

/**
 * Hide admin bar depending on role
 */
function u_hide_admin_bar() {
	if ( ! current_user_can( 'edit_posts' ) ) {
		show_admin_bar( false );
	}
}
add_action( 'set_current_user', 'u_hide_admin_bar' );
